...
pause_time = 0.001
p_create_random_obstacle = 0 # Allows random generation of obstacles: see original file for details
...
class DStarLite:
    ...
    def compare_keys(self, key_pair1: tuple,
                     key_pair2: tuple):
    ...
    def main(self, source_centroid, target_centroid, spoofed_ox=[], spoofed_oy=[]):
        
        # turn source and target agents into nodes (coordinates in pxl units)
        (sx,sy),(gx,gy)=self.floor_PxlObject.pixelate([source_centroid.coords[0], target_centroid.coords[0]])
        ...
        # prepare for next path calculation
        self.initialized = False
        return True, stepByStep_Path
